...
int RecombinationHistory::rhs_peebles_ode(double x, const double *Xe, double *dXedx){
  ...
  //=============================================================================
  const double T_b = TCMB/a
  const double lambda_alpha = H_of_x * (pow(3*epsilon_0,3.0)/(pow(8*Constants.pi,2.0)*pow(c*hbar,3.0))) ;
  ...
  const double beta = alpha_2 * pow((m_e * k_b * T_b) / (2 * pi * pow(hbar, 2.0)), 1.5) * exp(-epsilon_0 / (k_b * T_b));
  const double beta_2 = beta*exp(3*epsilon_0/(4*k_b*T_b))
  
  double Cr = (lambda_2s1s +) * (3 * pow(H_of_x, 2.0) / 8 * Constants.pi * G);
