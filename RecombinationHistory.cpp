...
std::pair<double,double> RecombinationHistory::electron_fraction_from_saha_equation(double x) const{
  ...
  const double nb  = OmegaB * (3 * pow(H_of_x, 2.0) / 8 * Constants.pi * G);
  const double saha_rhs  = (1 / nb) * pow(m_e * k_b * T_b / (2 * Constants.pi * pow(hbar, 2.0)), 1.5) * exp(-epsilon_0 / (k_b * T_b));
  
  Xe = -(saha_rhs / 2) * (1 - sqrt(1 + (4 / saha_rhs)));
  ne = Xe * nb;
...
int RecombinationHistory::rhs_peebles_ode(double x, const double *Xe, double *dXedx){
  ...
  //=============================================================================
  const double T_b = TCMB / a;
  const double n_H = (1 - Yp) * (3 * pow(H0, 2.0) * OmegaB / (8 * Constants.pi * G * m_H * pow(a, 3.0));
  const double n_1s = (1-X_e)*n_H
  const double lambda_alpha = H_of_x * (pow(3*epsilon_0,3.0)/(pow(8*Constants.pi,2.0)*pow(c*hbar,3.0)*n_1s));
  const double phi_2 = 0.448 * log(epsilon_0 / (k_b * T_b));
  const double alpha_2 = (8 / sqrt(3 * Constants.pi)) * c * sigma_T * (sqrt(epsilon_0 / k_b * T_b)) * phi_2;
  const double beta = alpha_2 * pow((m_e * k_b * T_b) / (2 * pi * pow(hbar, 2.0)), 1.5) * exp(3*epsilon_0 / (4 * k_b * T_b));
  const double beta_combined = alpha_2 * pow((m_e * k_b * T_b) / (2 * pi * pow(hbar, 2.0)), 1.5) * exp(-epsilon_0 / (4 * k_b * T_b));
  const double Cr = (lambda_2s1s + lambda_alpha)/ (lambda_2s1s + lambda_alpha + beta_combined);
  
  dXedx[0] = (Cr/H_of_x)*(beta*(1-X_e)-n_H*alpha_2*pow(X_e,2.0);
